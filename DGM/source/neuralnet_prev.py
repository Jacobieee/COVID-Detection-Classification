...
class DGM(object):
    ...
    def __encoder(self, x, ksize=3, outdim=1, reuse=False, \
        name='enc', activation='relu', norm=True, depth=5, verbose=True):

        with tf.variable_scope(name, reuse=reuse):

            c_in, c_out = self.channel, 64
            ...
            rs = tf.compat.v1.reshape(x, shape=[self.batch_size, int(7*7*1024)], \
                name="%s_rs" %(name))
    ...
    def __decoder(self, z, ksize=3, reuse=False, \
        name='dec', activation='relu', norm=True, depth=6, verbose=True):

        with tf.variable_scope(name, reuse=reuse):

            c_in, c_out = 1024, 1024
            h_out, w_out = 14, 14

            fc1 = self.layer.fully_connected(x=z, c_out=7*7*1024, \
                batch_norm=norm, training=self.training, \
                activation=activation, name="%s_fc1" %(name), verbose=verbose)
            rs = tf.compat.v1.reshape(fc1, shape=[self.batch_size, 7, 7, 1024], \
                name="%s_rs" %(name))
